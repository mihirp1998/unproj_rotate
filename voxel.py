...
def get_transform_matrix_tf_(theta, phi, invert_rot = False, invert_focal = False):
    ...
    if invert_focal:
        intrinsic_diag = [1.0, float(FOCAL_LENGTH_X), float(FOCAL_LENGTH_Y), 1.0]
    else:
        intrinsic_diag = [1.0, 1.0/float(FOCAL_LENGTH_X), 1.0/float(FOCAL_LENGTH_Y), 1.0]
...
def rotate_and_project_voxel(voxel, phi,theta):
    BS = getbs(voxel)
    S = getsize(voxel)
    rotmat = get_transform_matrix_tf(theta, phi)
    r = rank(voxel)
    assert r in [4,5]
    if r == 4:
...
def rotate_voxel(voxel, phi,theta):
    BS = getbs(voxel)
    S = getsize(voxel)
    # st()
    rotmat = get_transform_matrix_tf(theta, phi)
...
def project_voxel(voxel,fov,scale):
    fx = W / 2.0 * 1.0 / math.tan(fov * math.pi / 180 / 2)
    fy = fx
    focal_length = fx / (W / 2.0)
    FOCAL_LENGTH_X = focal_length
    FOCAL_LENGTH_Y = focal_length *scale

    # st()
    BS = getbs(voxel)
    S = getsize(voxel)
    # theta , phi
    rotmat = tf.constant(get_transform_matrix(0.0, 0.0,FOCAL_LENGTH_Y,FOCAL_LENGTH_X), dtype = tf.float32)
